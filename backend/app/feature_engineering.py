...
class FeatureEngineer:
    ...
    def __init__(self):
        ...
        self.continuous_skewed_features = [
            'trans_in_last_1h', 'trans_in_last_24h', 'trans_in_last_7d',
            'amt_per_card_avg_ratio_1h', 'amt_per_card_avg_ratio_24h', 'amt_per_card_avg_ratio_7d',
            'amt_per_category_avg_ratio_1h', 'amt_per_category_avg_ratio_24h', 'amt_per_category_avg_ratio_7d'
        ]
        
        self.cols_to_drop_ids = [
            'is_fraud', 'trans_datetime', 'cc_num', 'acct_num',
        ...
        try:
            ...
            for tx in transactions:
                ...
                row = {
                    ...
                    'category': transaction['merchant']['category'].lower().replace(' & ', '_').replace(' ', '_'),
                ...
                additional_features = [
                    'time_since_last_trans_seconds', 'trans_speed_kmh', 'hour_of_day', 
                    'is_late_night_fraud_window', 'is_late_evening_fraud_window', 
                    'day_of_week', 'amt_diff_from_card_median_1d'
                ]
                
                # Extract hour of day from timestamp
                row['hour_of_day'] = row['trans_datetime'].hour
                row['day_of_week'] = row['trans_datetime'].weekday()
                row['is_late_night_fraud_window'] = 1 if row['hour_of_day'] >= 23 or row['hour_of_day'] <= 5 else 0
            ...
            logger.info(f"✅ Converted {len(transactions)} transactions to DataFrame with {df.shape[1]} columns")
    ...
    def prepare_features(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        try:
            ...
            nan_fill_features = self.continuous_skewed_features + ['amt_diff_from_card_median_1d', 'amt_diff_from_card_median_7d']
            ...
            for feature in self.continuous_skewed_features:
                if feature in transformed_df.columns:
                    ...
                    transformed_df[new_log_feature_name] = np.log1p(transformed_df[feature].clip(lower=0))
    ...
    def process_transactions(self, transactions: List[Dict[str, Any]]) -> Tuple[pd.DataFrame, pd.Series]:
        """
