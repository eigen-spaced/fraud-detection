...
class OpenRouterService:
    ...
    async def explain_fraud_prediction(
        ...
        try:
            ...
            logger.info(f"Generated explanation for transaction {transaction_data.get('transaction_id', 'unknown')}")
            return explanation
...
"""
        ...
        risk_level = "HIGH" if fraud_probability > 0.75 else "MEDIUM" if fraud_probability > 0.45 else "LOW"
        prediction_info = f"""
Model Prediction:
...
Please provide a clear, professional explanation of why this transaction was flagged with this risk level. Focus on the most significant factors and what they mean in practical terms."""
    ...
    def _build_pattern_analysis_prompt(
        ...
        high_risk_count = sum(1 for p in predictions if p > 0.75)
        medium_risk_count = sum(1 for p in predictions if 0.45 <= p <= 0.75)
        ...
        for i, (tx, pred) in enumerate(zip(transactions, predictions)):
            if pred > 0.75 and len(high_risk_samples) < 3:
                high_risk_samples.append(f"- ${tx.get('amount', 'N/A')} at {tx.get('merchant_name', 'N/A')} ({pred:.2%} risk)")
