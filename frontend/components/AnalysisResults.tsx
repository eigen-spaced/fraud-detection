...
export default function AnalysisResults({ result, isLoading, error }: AnalysisResultsProps) {
  return (
    <div className="flex-1 overflow-y-auto p-4">
      {isLoading ? (
        <div className="flex flex-col items-center justify-center h-full gap-4">
          <svg className="animate-spin h-12 w-12 text-ocean-500" viewBox="0 0 24 24">
            <circle
          ...
          <p className="text-navy-700 text-lg font-medium">Analyzing transactions...</p>
          <p className="text-navy-500 text-sm">Running ML models and generating explanations</p>
        </div>
      ) : error ? (
        <div className="flex flex-col items-center justify-center h-full gap-4">
        ...
        <div className="flex flex-col items-center justify-center h-full gap-4 text-center">
          ...
          <p className="text-navy-500 text-sm max-w-md">
            Paste your transaction JSON in the input panel and click &ldquo;Analyze Transactions&rdquo; to get started
          </p>
        </div>
      ) : isRefusalResponse(result) ? (
        <div className="flex flex-col items-center justify-center h-full gap-4">
          <div className="text-6xl">🚫</div>
          <p className="text-coral-600 text-xl font-semibold">Request Refused</p>
        ...
        <div className="space-y-6">
          ...
          <div className="p-4 bg-navy-50 rounded-lg border border-navy-200">
            <h3 className="text-lg font-semibold text-navy-900 mb-2">Summary</h3>
          ...
          {result.warnings && result.warnings.length > 0 && (
            <div className="p-4 bg-golden-50 border border-golden-200 rounded-lg">
              <h3 className="text-sm font-semibold text-golden-700 mb-2">⚠️ Warnings</h3>
              <ul className="space-y-1">
                {result.warnings.map((warning, idx) => (
                  <li key={idx} className="text-golden-700 text-sm">• {warning}</li>
                ))}
              </ul>
            </div>
          )}
          ...
          {/* Individual Analyses */}
          <div>
            <h3 className="text-lg font-semibold text-navy-900 mb-3">Transaction Details</h3>
          ...
          {result.citations && result.citations.length > 0 && (
            <div className="p-4 bg-navy-50 rounded-lg border border-navy-200">
              <h3 className="text-sm font-semibold text-navy-700 mb-2">📚 Citations</h3>
